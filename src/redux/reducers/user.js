...
export const UserAccount = handleActions( {
  ...
  'request user auth group list'( state, action ) {
    return { ...state, loading: true }
  },
  'receive user auth group list'( state, action ) {
    const { req, res } = action.payload
    let authGPList = [];
    if ( hasResponseError( res ) ) {
      message.error( res.msg )
    ...
    const { data } = res;
    authGPList = res.data.list.map( item => ( { value: item.uObjectUUID, text: item.strGroupName } ) )
    // authGPList = fn_mes_array.addKey( res.data.list, 'key' );
    // state.defaultMenuKeys = [authGPList[0].strGroupCode];
    // const authGroupItem = authGPList[0];
    return {
        ...state, authGPList, loading: false,
    }
  },
